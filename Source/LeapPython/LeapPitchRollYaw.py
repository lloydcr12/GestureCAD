import Leap, sys, threading, time, pickle, csv
...
class LeapMotionListener(Leap.Listener):
    ...
    def on_frame(self, controller): #290fps leap data
        ...
        for hand in frame.hands:
            ...
            f = open(filename, 'w')
            f.write(str(direction.pitch * Leap.RAD_TO_DEG), str(normal.roll * Leap.RAD_TO_DEG), str(direction.yaw * Leap.RAD_TO_DEG))"""

            with open('D:\Source\LeapPython3\PitchRollYaw.csv', 'w') as csvfile:
                fieldnames = ['Pitch', 'Roll', 'Yaw']
                writer = csv.DictWriter(csvfile, fieldnames=fieldnames)

                writer.writeheader()
                writer.writerow({'Pitch': str(direction.pitch * Leap.RAD_TO_DEG), 'Roll': str(normal.roll * Leap.RAD_TO_DEG), 'Yaw': str(direction.yaw * Leap.RAD_TO_DEG)})
